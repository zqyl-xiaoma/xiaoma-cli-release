...
This document defines the standard for IDE-specific content injection in XIAOMA modules. Each IDE can inject its own specific content into XIAOMA templates during installation without polluting the source files with IDE-specific code. The installation process is interactive, allowing users to choose what IDE-specific features they want to install.
...
async processModuleInjections(projectDir, xiaomaDir, options) {
  for (const moduleName of options.selectedModules) {
    const configPath = path.join(
      xiaomaDir, 'src/modules', moduleName,
...
## Example: XMC Module with Claude Code
...
injections:
  ...
  - file: '{xiaoma_folder}/xmc/templates/prd.md'

...
class WorkflowCommandGenerator {
   ...
   */
  async generateWorkflowCommands(projectDir, xiaomaDir) {
    const workflows = await this.loadWorkflowManifest(xiaomaDir);
    ...
    const baseCommandsDir = path.join(projectDir, '.claude', 'commands', 'xiaoma');
    ...
    for (const workflow of standaloneWorkflows) {
      ...
      const commandContent = await this.generateCommandContent(workflow, xiaomaDir);
  ...
  async collectWorkflowArtifacts(xiaomaDir) {
    const workflows = await this.loadWorkflowManifest(xiaomaDir);
   ...
   */
  async generateCommandContent(workflow, xiaomaDir) {
    // Load the template
    const template = await fs.readFile(this.templatePath, 'utf8');

    // Convert source path to installed path
    // From: /Users/.../src/modules/xmc/workflows/.../workflow.yaml
    // To: {project-root}/{xiaoma_folder}/xmc/workflows/.../workflow.yaml
    ...
    if (workflowPath.includes('/src/modules/')) {
      ...
      if (match) {
        workflowPath = `${this.xiaomaFolderName}/${match[1]}`;
      }
    } else if (workflowPath.includes('/src/core/')) {
      const match = workflowPath.match(/\/src\/core\/(.+)/);
      if (match) {
        workflowPath = `${this.xiaomaFolderName}/core/${match[1]}`;
      }
    }
    ...
    return template
      ...
      .replaceAll('{{author}}', workflow.author || 'XIAOMA');
  }
...
1. LOAD {project-root}/${this.xiaomaFolderName}/core/tasks/workflow.xml
...
`;
  ...
  transformWorkflowPath(workflowPath) {
    ...
    if (workflowPath.includes('/src/modules/')) {
      const match = workflowPath.match(/\/src\/modules\/(.+)/);
      if (match) {
        transformed = `{project-root}/${this.xiaomaFolderName}/${match[1]}`;
      }
    } else if (workflowPath.includes('/src/core/')) {
      const match = workflowPath.match(/\/src\/core\/(.+)/);
      if (match) {
        transformed = `{project-root}/${this.xiaomaFolderName}/core/${match[1]}`;
      }
    }
  ...
  async loadWorkflowManifest(xiaomaDir) {
    const manifestPath = path.join(xiaomaDir, '_cfg', 'workflow-manifest.csv');

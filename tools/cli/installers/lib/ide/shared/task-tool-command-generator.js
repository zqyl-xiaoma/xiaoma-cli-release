...
class TaskToolCommandGenerator {
  ...
  async generateTaskToolCommands(projectDir, xiaomaDir, baseCommandsDir = null) {
    const tasks = await this.loadTaskManifest(xiaomaDir);
    const tools = await this.loadToolManifest(xiaomaDir);
    ...
    const commandsDir = baseCommandsDir || path.join(projectDir, '.claude', 'commands', 'xiaoma');
...
`;
   ...
   */
  async loadTaskManifest(xiaomaDir) {
    const manifestPath = path.join(xiaomaDir, '_cfg', 'task-manifest.csv');
   ...
   */
  async loadToolManifest(xiaomaDir) {
    const manifestPath = path.join(xiaomaDir, '_cfg', 'tool-manifest.csv');

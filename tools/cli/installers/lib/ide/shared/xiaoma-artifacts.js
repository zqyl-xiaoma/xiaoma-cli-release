 ...
 */
async function getAgentsFromBmad(xiaomaDir, selectedModules = []) {
  const agents = [];

  // Get core agents
  if (await fs.pathExists(path.join(xiaomaDir, 'core', 'agents'))) {
    const coreAgents = await getAgentsFromDir(path.join(xiaomaDir, 'core', 'agents'), 'core');
    agents.push(...coreAgents);
  }

  // Get module agents
  for (const moduleName of selectedModules) {
    const agentsPath = path.join(xiaomaDir, moduleName, 'agents');
...
async function getTasksFromBmad(xiaomaDir, selectedModules = []) {
  const tasks = [];

  if (await fs.pathExists(path.join(xiaomaDir, 'core', 'tasks'))) {
    const coreTasks = await getTasksFromDir(path.join(xiaomaDir, 'core', 'tasks'), 'core');
    tasks.push(...coreTasks);
  }

  for (const moduleName of selectedModules) {
    const tasksPath = path.join(xiaomaDir, moduleName, 'tasks');

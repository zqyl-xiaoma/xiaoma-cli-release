...
class Manifest {
  ...
  async create(xiaomaDir, data, installedFiles = []) {
    const manifestPath = path.join(xiaomaDir, '_cfg', 'manifest.yaml');
  ...
  async read(xiaomaDir) {
    const yamlPath = path.join(xiaomaDir, '_cfg', 'manifest.yaml');
    const yaml = require('js-yaml');
   ...
   */
  async addModule(xiaomaDir, moduleName) {
    const manifest = await this.read(xiaomaDir);
    if (!manifest) {
    ...
    if (!manifest.modules.includes(moduleName)) {
      ...
      await this.update(xiaomaDir, { modules: manifest.modules });
    }
  }

...
class DependencyResolver {
   ...
   */
  async resolve(xiaomaDir, selectedModules = [], options = {}) {
    if (options.verbose) {
    ...
    const primaryFiles = await this.collectPrimaryFiles(xiaomaDir, modulesToProcess);
    ...
    const resolvedDeps = await this.resolveDependencyPaths(xiaomaDir, allDependencies);

    // Fourth pass: check for transitive dependencies
    const transitiveDeps = await this.resolveTransitiveDependencies(xiaomaDir, resolvedDeps);

    // Combine all files
    const allFiles = new Set([...primaryFiles.map((f) => f.path), ...resolvedDeps, ...transitiveDeps]);

    // Organize by module
    const organizedFiles = this.organizeByModule(xiaomaDir, allFiles);
   ...
   */
  async collectPrimaryFiles(xiaomaDir, modules) {
    const files = [];

    for (const module of modules) {
      // Handle both source (src/) and installed (xiaoma/) directory structures
      let moduleDir;

      // Check if this is a source directory (has 'src' subdirectory)
      const srcDir = path.join(xiaomaDir, 'src');
      if (await fs.pathExists(srcDir)) {
        ...
        // Installed directory structure: xiaoma/core or xiaoma/modules/xxx
        moduleDir = module === 'core' ? path.join(xiaomaDir, 'core') : path.join(xiaomaDir, 'modules', module);
      }
  ...
  async parseDependencies(files) {
    ...
    for (const file of files) {
      ...
      for (const ref of fileRefs) {
        ...
        // Paths starting with xiaoma/ are absolute references to the xiaoma installation
        const depType = ref.startsWith('xiaoma/') ? 'xiaoma-path' : 'file';
        allDeps.add({
          from: file.path,
          dependency: ref,
          type: depType,
  ...
  parseCommandReferences(content) {
    ...
    const commandPattern = /@(task-|agent-|xiaoma-)([a-z0-9-]+)/g;
    let match;
    ...
    // Match file paths like xiaoma/core/agents/analyst
    const pathPattern = /xiaoma\/(core|xmc|cis)\/(agents|tasks)\/([a-z0-9-]+)/g;
  ...
  parseFileReferences(content) {
    ...
    while ((match = execPattern.exec(content)) !== null) {
      ...
      if (execPath && execPath !== '*') {
        ...
        // Usage is like {project-root}/xiaoma/core/tasks/foo.md
   ...
   */
  async resolveDependencyPaths(xiaomaDir, dependencies) {
    const resolved = new Set();

    for (const dep of dependencies) {
      const resolvedPaths = await this.resolveSingleDependency(xiaomaDir, dep);
   ...
   */
  async resolveSingleDependency(xiaomaDir, dep) {
    const paths = [];
    ...
    switch (dep.type) {
      ...
      case 'file': {
        ...
        if (depPath.includes('{project-root}')) {
          // Remove {project-root} and resolve as xiaoma path
          depPath = depPath.replace('{project-root}', '');

          if (depPath.startsWith('xiaoma/')) {
            const xiaomaPath = depPath.replace(/^xiaoma\//, '');

            // Handle glob patterns
            if (depPath.includes('*')) {
              // Extract the base path and pattern
              const pathParts = xiaomaPath.split('/');
              ...
              let basePath;
              if (module === 'core') {
                basePath = path.join(xiaomaDir, 'core', middlePath);
              } else {
                basePath = path.join(xiaomaDir, 'modules', module, middlePath);
              }
              ...
              // Direct path
              if (xiaomaPath.startsWith('core/')) {
                const corePath = path.join(xiaomaDir, xiaomaPath);
                if (await fs.pathExists(corePath)) {
                  paths.push(corePath);
                }
              } else {
                const parts = xiaomaPath.split('/');
                const module = parts[0];
                const rest = parts.slice(1).join('/');
                const modulePath = path.join(xiaomaDir, 'modules', module, rest);
      ...
      case 'command': {
        ...
        const commandPath = await this.resolveCommandToPath(xiaomaDir, dep.dependency);
        if (commandPath) {
      ...
      case 'xiaoma-path': {
        // Resolve xiaoma/ paths (from {project-root}/xiaoma/... references)
        // These are paths relative to the src directory structure
        const xiaomaPath = dep.dependency.replace(/^xiaoma\//, '');

        // Try to resolve as if it's in src structure
        // xiaoma/core/tasks/foo.md -> src/core/tasks/foo.md
        // xiaoma/xmc/tasks/bar.md -> src/modules/xmc/tasks/bar.md
        ...
        if (xiaomaPath.startsWith('core/')) {
          ...
          // It's a module path like xmc/tasks/foo.md or cis/agents/bar.md
          const parts = xiaomaPath.split('/');
          const module = parts[0];
          const rest = parts.slice(1).join('/');
          const modulePath = path.join(xiaomaDir, 'modules', module, rest);
      ...
      case 'template': {
        ...
        if (templateDep.includes('{project-root}')) {
          // Remove {project-root} and treat as xiaoma-path
          templateDep = templateDep.replace('{project-root}', '');

          // Now resolve as a xiaoma path
          if (templateDep.startsWith('xiaoma/')) {
            const xiaomaPath = templateDep.replace(/^xiaoma\//, '');
   ...
   */
  async resolveCommandToPath(xiaomaDir, command) {
    // Parse command format: @task-name or @agent-name or xiaoma/module/type/name

    if (command.startsWith('@task-')) {
      const taskName = command.slice(6);
      // Search all modules for this task
      for (const module of ['core', 'xmc', 'cis']) {
        const taskPath =
          module === 'core'
            ? path.join(xiaomaDir, 'core', 'tasks', `${taskName}.md`)
            : path.join(xiaomaDir, 'modules', module, 'tasks', `${taskName}.md`);
      ...
      for (const module of ['core', 'xmc', 'cis']) {
        const agentPath =
          module === 'core'
            ? path.join(xiaomaDir, 'core', 'agents', `${agentName}.md`)
            : path.join(xiaomaDir, 'modules', module, 'agents', `${agentName}.md`);
        if (await fs.pathExists(agentPath)) {
          return agentPath;
        }
      }
    } else if (command.startsWith('xiaoma/')) {
      ...
      if (parts.length >= 4) {
        ...
        const filePath =
          module === 'core' ? path.join(xiaomaDir, 'core', type, fileName) : path.join(xiaomaDir, 'modules', module, type, fileName);
   ...
   */
  async resolveTransitiveDependencies(xiaomaDir, directDeps) {
        ...
        const subDeps = await this.parseDependencies([
          {
            path: depPath,
            type: 'dependency',
            module: this.getModuleFromPath(xiaomaDir, depPath),
            name: path.basename(depPath),
          },
        ]);

        const resolvedSubDeps = await this.resolveDependencyPaths(xiaomaDir, subDeps);
   ...
   */
  getModuleFromPath(xiaomaDir, filePath) {
    const relative = path.relative(xiaomaDir, filePath);
   ...
   */
  organizeByModule(xiaomaDir, files) {
    const organized = {};

    for (const file of files) {
      const module = this.getModuleFromPath(xiaomaDir, file);
      if (!organized[module]) {
        organized[module] = {
          agents: [],
          tasks: [],
      ...
      if (file.includes('/src/core/') || file.includes('/src/modules/')) {
        moduleBase = module === 'core' ? path.join(xiaomaDir, 'src', 'core') : path.join(xiaomaDir, 'src', 'modules', module);
      } else {
        // Installed structure
        moduleBase = module === 'core' ? path.join(xiaomaDir, 'core') : path.join(xiaomaDir, 'modules', module);
      }

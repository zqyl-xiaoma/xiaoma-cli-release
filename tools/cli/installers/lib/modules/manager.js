...
class ModuleManager {
  ...
  /**
   * Copy a file and replace {xiaoma_folder} placeholder with actual folder name
  ...
  async install(moduleName, xiaomaDir, fileTrackingCallback = null, options = {}) {
    ...
    if (!options.skipModuleInstaller) {
      await this.runModuleInstaller(moduleName, xiaomaDir, options);
    }
   ...
   */
  async update(moduleName, xiaomaDir, force = false) {
    const sourcePath = path.join(this.modulesSourcePath, moduleName);
    const targetPath = path.join(xiaomaDir, moduleName);
    ...
    if (force) {
      ...
      await fs.remove(targetPath);
      return await this.install(moduleName, xiaomaDir);
    } else {
  ...
  async copyWorkflowYamlStripped(sourceFile, targetFile) {
    ...
    // IMPORTANT: Replace {xiaoma_folder} BEFORE parsing YAML
    // Otherwise parsing will fail on the placeholder
    yamlContent = yamlContent.replaceAll('{xiaoma_folder}', this.xiaomaFolderName);
  ...
  async vendorCrossModuleWorkflows(sourcePath, targetPath, moduleName) {
    ...
    for (const agentFile of yamlFiles) {
      ...
      for (const item of workflowInstallItems) {
        ...
        // Handle both {xiaoma_folder} placeholder and hardcoded 'xiaoma'
        // Example: {project-root}/{xiaoma_folder}/xmc/workflows/4-implementation/create-story/workflow.yaml
        // Or: {project-root}/xiaoma/xmc/workflows/4-implementation/create-story/workflow.yaml
  ...
  async updateWorkflowConfigSource(workflowYamlPath, newModuleName) {
    ...
    // Replace config_source: "{project-root}/{xiaoma_folder}/OLD_MODULE/config.yaml"
    // with config_source: "{project-root}/{xiaoma_folder}/NEW_MODULE/config.yaml"
    // Note: At this point {xiaoma_folder} has already been replaced with actual folder name
    const configSourcePattern = /config_source:\s*["']?\{project-root\}\/[^/]+\/[^/]+\/config\.yaml["']?/g;
    const newConfigSource = `config_source: "{project-root}/${this.xiaomaFolderName}/${newModuleName}/config.yaml"`;

    const updatedYaml = yamlContent.replaceAll(configSourcePattern, newConfigSource);

    if (updatedYaml !== yamlContent) {
      await fs.writeFile(workflowYamlPath, updatedYaml, 'utf8');
      console.log(chalk.dim(`      Updated config_source to: ${this.xiaomaFolderName}/${newModuleName}/config.yaml`));
    }
  }
   ...
   */
  async runModuleInstaller(moduleName, xiaomaDir, options = {}) {
    ...
    try {
      ...
      if (typeof moduleInstaller.install === 'function') {
        // Get project root (parent of xiaoma directory)

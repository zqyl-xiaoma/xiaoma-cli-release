...
The XiaoMa CLI handles installation and web bundling for the XIAOMA-CLI framework. It compiles YAML agents into two distinct formats: **IDE-integrated agents** (filesystem-aware, customizable) and **web bundles** (self-contained, dependency-embedded).

## Table of Contents

- [XiaoMa CLI Tool](#xiaoma-cli-tool)
...
# Direct CLI usage
node tools/cli/xiaoma-cli.js install --target /path/to/project --modules xmc,xmb --ides codex

# Flags:
#   --target <path>        Target project directory
#   --modules <list>       Comma-separated: xmc, xmb, cis
...
node tools/cli/bundlers/bundle-web.js module xmc       # One module
node tools/cli/bundlers/bundle-web.js agent xmc pm     # One agent
```

### Utilities

```bash
npm run xiaoma:status              # Installation status
...
3. Install Core + Modules
   - Copy files to {target}/{xiaoma_folder}/
...
1. **Module-Level**: `src/modules/{module}/_module-installer/platform-specifics/{ide}.js`
   ...
   - Example: XMC creates subagents when installed with Claude Code

2. **IDE-Level**: Embedded in IDE handler's `createArtifacts()` method
   - IDE provides custom handling for specific modules
   - Example: Windsurf configures cascade workflows for XMC
...
The installer generates **5 manifest files** in `{target}/{xiaoma_folder}/_cfg/`:
...
- Auto-backup to `.xiaoma-backup-{timestamp}` if selected
...
agent:
  ...
  menu:
    - trigger: '*create-brief'
      workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/.../workflow.yaml'
```
...
<agent id="..." name="PM">
  <activation critical="MANDATORY">
    <step n="2">Load {project-root}/{xiaoma_folder}/xmc/config.yaml at runtime</step>
    ...
  </activation>
  <persona>...</persona>
  <menu>...</menu>
...
<agent id="..." name="PM">
  ...
  <menu>...</menu>
  <bundled-files>
    <file id="xiaoma/xmc/config.yaml"><![CDATA[...]]></file>
    <file id="xiaoma/xmc/workflows/.../workflow.yaml"><![CDATA[...]]></file>
    ...
  </bundled-files>
</agent>
````
...
| ----------------------- | -------------------------------------- | --------------------------------- |
| **Trigger**             | `npm run install:xiaoma`               | `npm run bundle`                  |
...
| **Output Location**     | `{target}/{xiaoma_folder}/` + IDE dirs | `web-bundles/`                    |
...
# Test installation
node tools/cli/xiaoma-cli.js install --target ./test-project --modules xmc --ides codex

# Test bundling
node tools/cli/bundlers/bundle-web.js agent xmc pm
...
- **XMC Workflows**: `src/modules/xmc/workflows/README.md`
- **Module Structure**: `src/modules/xmb/workflows/create-module/module-structure.md`
- **Agent Creation**: `src/modules/xmb/workflows/create-agent/README.md`

---

## Support

- **Issues**: https://github.com/xiaoma-code-org/XIAOMA-CLI/issues
- **Discord**: https://discord.gg/gk8jAdXWmj (#general-dev, #bugs-issues)
- **YouTube**: https://www.youtube.com/@XiaoMaCode

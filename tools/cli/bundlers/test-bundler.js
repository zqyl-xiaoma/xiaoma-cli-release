...
async function testWebBundler() {
  ...
  try {
    const result = await bundler.bundleAgent('xmc', 'analyst.md');

    // Check if bundle was created
    const bundlePath = path.join(bundler.outputDir, 'xmc', 'agents', 'analyst.xml');
    if (await fs.pathExists(bundlePath)) {
      ...
      const hasManifests =
        content.includes('<agent-party id="xiaoma/_cfg/agent-manifest.csv">') &&
        content.includes('<manifest id="xiaoma/web-manifest.xml">');
  ...
  try {
    const result = await bundler.bundleAgent('xmc', 'architect.md');
    const bundlePath = path.join(bundler.outputDir, 'xmc', 'agents', 'architect.xml');
  ...
  try {
    const results = await bundler.bundleModule('xmc');
    console.log(chalk.green(`✓ Bundled ${results.agents.length} agents from xmc module`));
    passedTests++;
  } catch (error) {
    console.error(chalk.red('✗ Failed to bundle xmc module:'), error.message);
    failedTests++;
  }

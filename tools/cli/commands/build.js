  ...
  action: async (agentName, options) => {
    try {
      ...
      if (!claudeDir) {
        ...
        console.log(chalk.dim('\nExample: npx xiaoma-cli build pm --directory /path/to/project'));
        process.exit(1);
      }
...
async function buildAgent(projectDir, agentName, force = false) {
  // First check standalone agents in xiaoma/agents/{agentname}/
  const standaloneAgentDir = path.join(projectDir, 'xiaoma', 'agents', agentName);
  ...
  if (await fs.pathExists(standaloneYamlPath)) {
    ...
    const customizePath = path.join(projectDir, 'xiaoma', '_cfg', 'agents', `${agentName}.customize.yaml`);
  ...
  // Find the agent YAML file in .claude/commands/xiaoma/
  const xiaomaCommandsDir = path.join(projectDir, '.claude', 'commands', 'xiaoma');

  // Search all module directories for the agent
  const modules = await fs.readdir(xiaomaCommandsDir);
  let found = false;

  for (const module of modules) {
    const agentYamlPath = path.join(xiaomaCommandsDir, module, 'agents', `${agentName}.agent.yaml`);
    const outputPath = path.join(xiaomaCommandsDir, module, 'agents', `${agentName}.md`);
...
async function buildAllAgents(projectDir, force = false) {
  ...
  // First, build standalone agents in xiaoma/agents/
  const standaloneAgentsDir = path.join(projectDir, 'xiaoma', 'agents');
  ...
  // Then, build module agents in .claude/commands/xiaoma/
  const xiaomaCommandsDir = path.join(projectDir, '.claude', 'commands', 'xiaoma');
  if (await fs.pathExists(xiaomaCommandsDir)) {
    ...
    for (const module of modules) {
      const agentsDir = path.join(xiaomaCommandsDir, module, 'agents');
...
async function checkBuildStatus(projectDir) {
  ...
  // Check standalone agents in xiaoma/agents/
  const standaloneAgentsDir = path.join(projectDir, 'xiaoma', 'agents');
  if (await fs.pathExists(standaloneAgentsDir)) {
    const agentDirs = await fs.readdir(standaloneAgentsDir);
  ...
  // Check module agents in .claude/commands/xiaoma/
  const xiaomaCommandsDir = path.join(projectDir, '.claude', 'commands', 'xiaoma');
  if (await fs.pathExists(xiaomaCommandsDir)) {
    const modules = await fs.readdir(xiaomaCommandsDir);
  ...
  if (needsRebuild.length === 0) {
    ...
    console.log(chalk.dim('\nRun "xiaoma build --all" to rebuild all agents'));
  }

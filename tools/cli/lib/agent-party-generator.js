...
<manifest id="xiaoma/_cfg/agent-manifest.csv" version="1.0" generated="${new Date().toISOString()}">
  <description>
...
`;
    ...
    for (const [module, agents] of Object.entries(agentsByModule)) {
      ...
      const moduleTitle =
        module === 'xmc' ? 'XMC Module' : module === 'cis' ? 'CIS Module' : module === 'core' ? 'Core Module' : 'Custom Module';
  ...
  extractAgentDetails(content, moduleName, agentName) {
    try {
      ...
      return {
        id: `xiaoma/${moduleName}/agents/${agentName}.md`,

...
```
Read: {project-root}/{xiaoma_folder}/xmb/reference/workflows/meal-prep-nutrition/workflow.md
```
...
- **Linear**: Like Meal Prep Plan (Init → Profile → Assessment → Strategy → Shopping → Prep)
  - See: `{project-root}/{xiaoma_folder}/xmb/reference/workflows/meal-prep-nutrition/`
...
- For xmb module: Workflows go to `{custom_workflow_location}` (defaults to `{xiaoma_folder}/custom/src/workflows`)

...
- ✓ Use core variables ({xiaoma_folder}, {output_folder}, etc.)
...
agent:
  ...
  menu:
    - trigger: implement-story
      workflow: '{xiaoma_folder}/xmc/workflows/dev-story/workflow.yaml'
      description: Implement user story

    - trigger: refactor
      workflow: '{xiaoma_folder}/xmc/workflows/refactor/workflow.yaml'
...
- ✓ Coordinates with other XMC agents
...
- A Simple agent COULD live in `.xiaoma/xmc/agents/`
...
1. **Designed FOR** a particular module ecosystem (XMC, CIS, XMB, etc.)
...
**Simple Agent added to XMC:**

- Lives in `.xiaoma/xmc/agents/formatter.agent.yaml`
- Bundled with XMC for convenience
- But still stateless, self-contained
- NOT a "Module Agent" - just a Simple agent in a module

**Module Agent in XMC:**

- Lives in `.xiaoma/xmc/agents/tech-writer.agent.yaml`
- Orchestrates XMC documentation workflows
- Coordinates with other XMC agents (PM, Dev, Analyst)
- Included in default XMC bundle
- IS a "Module Agent" - designed for XMC ecosystem
...
- Designed FOR a specific module ecosystem (XMC, CIS, etc.)

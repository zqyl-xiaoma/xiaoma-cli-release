...
- Module-specific functionality (XMC, XMB, CIS, custom modules)
...
```
.xiaoma/{module-code}/agents/{agent-name}.md
```
...
agent:
  metadata:
    id: '{xiaoma_folder}/{module-code}/agents/{agent-name}.md'
    name: 'Persona Name'
  ...
  menu:
    - trigger: workflow-action
      workflow: '{project-root}/{xiaoma_folder}/{module-code}/workflows/{workflow-name}/workflow.yaml'
      description: 'Execute module workflow'

    - trigger: another-workflow
      workflow: '{project-root}/{xiaoma_folder}/core/workflows/{workflow-name}/workflow.yaml'
      description: 'Execute core workflow'

    - trigger: task-action
      exec: '{project-root}/{xiaoma_folder}/{module-code}/tasks/{task-name}.xml'
      description: 'Execute module task'

    - trigger: cross-module
      workflow: '{project-root}/{xiaoma_folder}/other-module/workflows/{workflow-name}/workflow.yaml'
      description: 'Execute workflow from another module'

    - trigger: with-template
      exec: '{project-root}/{xiaoma_folder}/core/tasks/create-doc.xml'
      tmpl: '{project-root}/{xiaoma_folder}/{module-code}/templates/{template-name}.md'
      description: 'Create document from template'

    - trigger: with-data
      exec: '{project-root}/{xiaoma_folder}/{module-code}/tasks/{task-name}.xml'
      data: '{project-root}/{xiaoma_folder}/_cfg/agent-manifest.csv'
...
- **id**: Path with `{xiaoma_folder}` variable (resolved at install time)
- **name**: Agent persona name
- **title**: Professional role
- **icon**: Single emoji
- **module**: Module code (xmc, xmb, cis, custom)
...
menu:
  - trigger: create-prd
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/prd/workflow.yaml'
    description: 'Create Product Requirements Document'
```

Invokes XIAOMA workflow engine to execute multi-step processes.
...
menu:
  - trigger: validate
    exec: '{project-root}/{xiaoma_folder}/core/tasks/validate-workflow.xml'
...
menu:
  - trigger: create-brief
    exec: '{project-root}/{xiaoma_folder}/core/tasks/create-doc.xml'
    tmpl: '{project-root}/{xiaoma_folder}/xmc/templates/brief.md'
...
menu:
  - trigger: team-standup
    exec: '{project-root}/{xiaoma_folder}/xmc/tasks/standup.xml'
    data: '{project-root}/{xiaoma_folder}/_cfg/agent-manifest.csv'
...
menu:
  - trigger: advanced-elicitation
    exec: '{project-root}/{xiaoma_folder}/core/tasks/advanced-elicitation.xml'
    description: 'Advanced elicitation techniques'
    web-only: true # Only shows in web bundle

  - trigger: git-operations
    exec: '{project-root}/{xiaoma_folder}/xmc/tasks/git-flow.xml'
...
- `{xiaoma_folder}` - XIAOMA installation folder (usually `.xiaoma`)
...
# BAD
workflow: "/Users/john/project/.xiaoma/xmc/workflows/prd/workflow.yaml"

# BAD
workflow: "../../../xmc/workflows/prd/workflow.yaml"
```
...
```xml
<step n="2">Load and read {project-root}/{xiaoma_folder}/{module}/config.yaml...</step>
```

## Reference Examples

See: `src/modules/xmc/agents/`
...
menu:
  - trigger: init
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/workflow-init/workflow.yaml'
    description: 'Initialize workflow path (START HERE)'

  - trigger: status
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/workflow-status/workflow.yaml'
    description: 'Check current workflow status'

  - trigger: next-step
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/next-step/workflow.yaml'
...
menu:
  # Phase 1: Analysis
  - trigger: brainstorm
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/1-analysis/brainstorm/workflow.yaml'
    description: 'Guided brainstorming session'

  - trigger: research
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/1-analysis/research/workflow.yaml'
    description: 'Market and technical research'

  # Phase 2: Planning
  - trigger: prd
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/2-planning/prd/workflow.yaml'
    description: 'Create PRD'

  - trigger: architecture
    workflow: '{project-root}/{xiaoma_folder}/xmc/workflows/2-planning/architecture/workflow.yaml'
...
menu:
  - trigger: party-mode
    workflow: '{project-root}/{xiaoma_folder}/core/workflows/party-mode/workflow.yaml'
    description: 'Bring all agents together'

  - trigger: brainstorm
    workflow: '{project-root}/{xiaoma_folder}/cis/workflows/brainstorming/workflow.yaml'
    description: 'Use CIS brainstorming techniques'
```

## Best Practices

1. **Use {xiaoma_folder} paths** - Portable across installations
...
menu:
  - trigger: start
    workflow: '{project-root}/{xiaoma_folder}/{module}/workflows/init/workflow.yaml'
...
menu:
  - trigger: status
    workflow: '{project-root}/{xiaoma_folder}/{module}/workflows/status/workflow.yaml'
...
| ------------- | ---------------------------------- | -------------------------------- |
| Location      | `{xiaoma_folder}/{module}/agents/` | `{xiaoma_folder}/custom/agents/` |
...
- [ ] id uses `{xiaoma_folder}/{module}/agents/{name}.md`
- [ ] All workflow paths use `{project-root}/{xiaoma_folder}/` prefix
- [ ] No hardcoded paths

...
agent:
  metadata:
    id: .xiaoma/agents/{agent-name}/{agent-name}.md
    name: 'Persona Name'
    title: 'Agent Title'
    icon: 'emoji'
    type: simple
...
- **id**: Final compiled path (`.xiaoma/agents/{name}/{name}.md` for standalone)
...
Questions asked during `xiaoma agent-install`:
...
See: `src/modules/xmb/reference/agents/simple-examples/commit-poet.agent.yaml`
...
cp /path/to/commit-poet.agent.yaml .xiaoma/custom/agents/

# Install with personalization
xiaoma agent-install
# or: npx xiaoma-cli agent-install
```

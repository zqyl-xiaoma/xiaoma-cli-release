 ...
 * Called by XIAOMA installer when processing the module
 */
...
async function setupConfigurations(config) {
  ...
  const configPath = path.join(config.project_root, 'xiaoma', config.module_code, 'config.json');
...
// Export functions for XIAOMA installer
module.exports = {
  installModule,
  initDatabase,
  generateSamples,

...
<critical>You MUST have already loaded and processed: {project-root}/{xiaoma_folder}/xmb/workflows/create-module/workflow.yaml</critical>
<critical>Study existing modules in: {project-root}/{xiaoma_folder}/ for patterns</critical>
...
<check if="create">
  <action>Invoke module-brief workflow: {project-root}/{xiaoma_folder}/xmb/workflows/module-brief/workflow.yaml</action>
...
2. **Module code** - Generate kebab-case from name following patterns:
   ...
   - Present suggested code and confirm it works for paths like {xiaoma_folder}/{{code}}/agents/
...
<critical>This is the STANDARD format that XIAOMA installer uses</critical>
...
# EXAMPLE Static path:
# data_path:
#   result: "{project-root}/{xiaoma_folder}/{{module_code}}/data"
...
The module can be configured in `{xiaoma_folder}/{{module_code}}/config.yaml`

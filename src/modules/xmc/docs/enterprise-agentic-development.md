# Enterprise Agentic Development with XiaoMa Method
...
- [How XiaoMa Method Enables PM/UX Technical Evolution](#how-xiaoma-cli-enables-pmux-technical-evolution)
...
**Impact:** Single developer with XiaoMa Method can deliver in 1 day what previously required full team and multiple sprints.
...
**The Bottom Line:** By 2026, successful PMs/UX will fluently operate in both vision and execution. **XiaoMa Method provides the structured framework to make this transition.**

---

## How XiaoMa Method Enables PM/UX Technical Evolution

XiaoMa Method is specifically designed to position PMs and UX designers for this future.
...
**Why it matters:** Traditional PRDs are human-readable prose. XiaoMa PRDs are **AI-executable requirement specifications**.
...
```bash
xiaoma architect *create-architecture
```
...
```bash
xiaoma pm *create-epics-and-stories
```
...
create-epics-and-stories generates story queue (informed by architecture)
   ↓
Stories loaded by human developers + XiaoMa agents
   ↓
...
- **XiaoMa Agentic:** PM writes PRD → AI agents implement → review PRs → deploy (2-5 days)
...
**UX Value:** Designs validated through working prototypes, not static mocks. Technical understanding built through XiaoMa workflows.

### 6. PM Technical Skills Development

**XiaoMa teaches PMs technical skills through:**
...
- XiaoMa asks about scale, performance, integrations
- PM answers business questions
- XiaoMa explains technical implications
...
**XiaoMa Agentic Model:**

- 1 PM → writes XiaoMa PRD → 20-50 AI agents execute stories in parallel → delivers 5-10 features/quarter
...
**PM Workflow (with XiaoMa + Cursor/v0):**

1. Use XiaoMa to generate PRD structure and requirements
2. Extract key user flow from PRD
3. Feed to Cursor/v0 with XiaoMa context
...
**XiaoMa Agentic:** Hours of development to validate idea

### 10. Career Path Evolution

**XiaoMa positions PMs for emerging roles:**
...
**Hiring advantage:** PMs using XiaoMa demonstrate:
...
**Problem:** Teams customize XiaoMa (agents, workflows, configs) but don't want personal tooling in main repo.

**Anti-pattern:** Adding `{xiaoma_folder}/` to `.gitignore` breaks IDE tools, submodule management.

### The Solution: Git Submodules

**Benefits:**

- XiaoMa exists in project but tracked separately
- Each developer controls their own XiaoMa version/config
...
git submodule add https://github.com/your-org/xmc-config.git xiaoma
git commit -m "Add XMC as submodule"
```
...
# Make personal customizations in {xiaoma_folder}/
```
...
```bash
cd /path/to/your-project
# XiaoMa available at ./{xiaoma_folder}/, load agents normally
```
...
**XMC Setup:** Direct installation, both use Claude Code, minimal customization
...
**XMC Setup:** Git submodule, team config repo, mix of Claude Code/Cursor users
...
**XMC Setup:** Each team has own submodule config, org-wide base config, variety of IDE tools
...
**Git submodules:** Best practice for XMC management across teams
**Team flexibility:** Mix of tools (Claude Code, Cursor, Windsurf) with shared XMC foundation
**Scalable patterns:** Epic-based, layer-based, feature-based distribution strategies

### The Future (2026)

PMs write XiaoMa PRDs → Stories auto-fed to cloud AI agents → Parallel implementation → Human review of PRs → Continuous deployment
...
- [Agents Guide](agents-guide.md) - Understanding XiaoMa agents

---

_XiaoMa Method fundamentally changes how PMs work, how teams structure work, and how products get built. Understanding these patterns is essential for enterprise success in the age of AI agents._

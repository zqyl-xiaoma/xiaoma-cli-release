# XiaoMa Method Scale Adaptive System
...
XiaoMa Method adapts to three distinct planning tracks:

- **Quick Flow**: Tech-spec only, implement immediately
- **XiaoMa Method**: PRD + Architecture, structured approach
...
| **XiaoMa Method**     | PRD + Arch + UX       | Products, platforms, complex features      |
...
flowchart TD
    ...
    START -->|Product, platform, complex| M[XiaoMa Method<br/>PRD + Architecture]
...
- **XiaoMa Method**: product, platform, dashboard, complex, multiple features
...
```
workflow-init: "Based on 'Add user dashboard with analytics', I recommend XiaoMa Method.
...
### Track 2: XiaoMa Method (RECOMMENDED)
...
_Detailed flowchart showing all phases, workflows, agents (color-coded), and decision points for the XiaoMa Method track. Each colored box represents a different agent role._
...
Includes all XiaoMa Method documents PLUS:
...
| **XiaoMa Method** | Recommended | PRD + UX  | Required     | None         | 10-50+          |
...
### Scenario 3: Customer Portal (XiaoMa Method)
...
### Scenario 4: E-commerce Platform (XiaoMa Method)
...
If `workflow-init` suggests XiaoMa Method, there's probably complexity you haven't considered. Review carefully before overriding.
...
For XiaoMa Method and Enterprise, implementation readiness checks prevent costly mistakes. Invest the time.

### 5. Architecture is Optional but Recommended for Brownfield

Brownfield XiaoMa Method makes architecture optional, but it's highly recommended. It distills complex codebase into focused solution design.
...
Product Brief is only offered for greenfield XiaoMa Method and Enterprise. It's optional but helps with strategic thinking.
...
- Clear track purposes:
  ...
  - XiaoMa Method = Product + system design

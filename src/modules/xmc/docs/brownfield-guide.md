# XiaoMa Method Brownfield Development Guide
...
| **XiaoMa Method**     | Feature sets, integrations | 10-50+          | Integrate with existing architecture            |
...
- Keywords: "fix", "bug" → Quick Flow, "dashboard", "platform" → XiaoMa Method, "enterprise", "multi-tenant" → Enterprise Method
...
```
System: "Found PRD.md (XiaoMa Method track, 30 stories, 6 months old)"
...
1. **First:** Run `shard-doc` tool to split large files:
   ...
   ```bash
   # Load XiaoMa Master or any agent
   {xiaoma_folder}/core/tools/shard-doc.xml --input docs/massive-doc.md
   ```
...
2. **Then:** Run `index-docs` task to create navigation:
   ...
   ```bash
   {xiaoma_folder}/core/tasks/index-docs.xml --directory ./docs
   ```
...
{xiaoma_folder}/core/tools/shard-doc.xml \
...
- **PRD** (XiaoMa Method) can't reference existing code → Designs incompatible features
...
- `product-brief` - Strategic planning (XiaoMa Method/Enterprise tracks only)
...
See the [Workflows section in XMC README](../README.md) for details.
...
**XiaoMa Method/Enterprise:** Use `prd` workflow
...
### Phase 3: Solutioning (XiaoMa Method/Enterprise Only)
...
- `create-architecture` - Extend architecture docs (XiaoMa Method/Enterprise)
- `create-epics-and-stories` - Create epics and stories AFTER architecture
- `implementation-readiness` - Validate before implementation (XiaoMa Method/Enterprise)
...
### Scenario 3: Feature Set (XiaoMa Method)
...
### Scenario 4: Complex Integration (XiaoMa Method)
...
product-brief           # Strategic planning (XiaoMa Method/Enterprise only)

# Phase 2: Planning (Required)
# PM agent:
tech-spec               # Quick Flow track
prd                     # XiaoMa Method/Enterprise tracks

# Phase 3: Solutioning (XiaoMa Method/Enterprise)
# Architect agent:
...
- `docs/xmc-workflow-status.yaml` - Progress tracker

**Phase 2 Planning:**

- `docs/tech-spec.md` (Quick Flow track)
- `docs/PRD.md` (XiaoMa Method/Enterprise tracks - FRs/NFRs only)
...
- `docs/architecture.md` (XiaoMa Method/Enterprise tracks)
...
flowchart TD
    ...
    TRACK -->|XiaoMa Method| PRD[prd → architecture]
...
5. ✅ **Start with simpler track if unsure** - Easy to upgrade (Quick Flow → XiaoMa Method)
...
- **[Quick Start Guide](quick-start.md)** - Getting started with XMC
...
- [GitHub Issues](https://github.com/xiaoma-code-org/XIAOMA-CLI/issues)
- [YouTube Channel](https://www.youtube.com/@XiaoMaCode)

**Documentation:**

- [Test Architect Guide](test-architecture.md) - Comprehensive testing strategy
- [XMC Module README](../README.md) - Complete module and workflow reference

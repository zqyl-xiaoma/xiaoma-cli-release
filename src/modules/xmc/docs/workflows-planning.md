# XMC Planning Workflows (Phase 2)
...
- Planning: Same as XiaoMa Method (`prd` workflow)
...
| **prd**              | PM          | XiaoMa Method, Enterprise | Strategic PRD with FRs/NFRs (no epic breakdown) | 10-50+          |
| **create-ux-design** | UX Designer | XiaoMa Method, Enterprise | Optional UX specification (after PRD)           | N/A             |
...
XMC uses three distinct planning tracks that adapt to project complexity:
...
**Planning (Phase 2):** Uses XiaoMa Method planning (PRD with FRs/NFRs)
...
**Purpose:** Strategic PRD with Functional Requirements (FRs) and Non-Functional Requirements (NFRs) for software products (XiaoMa Method track).
...
**Purpose:** UX specification for projects where user experience is the primary differentiator (XiaoMa Method track).
...
- **Software product** → `prd` (XiaoMa Method)
- **UX innovation project** → `create-ux-design` + `prd` (XiaoMa Method)
...
| PRD.md          | **architecture** (Level 3-4)       | XiaoMa Method (recommended)  |
| ux-spec.md      | **architecture** (frontend design) | XiaoMa Method                |
...
- **XiaoMa Method:** Optional Phase 3 (simple), Required Phase 3 (complex)
...
If `workflow-init` suggests XiaoMa Method, there's likely complexity you haven't considered. Review carefully before overriding.
...
### Greenfield Software (XiaoMa Method)
...
### Brownfield Software (XiaoMa Method)
...
A: No. Simple changes use `tech-spec` (Quick Flow). Only XiaoMa Method and Enterprise tracks create PRDs.

**Q: Can I skip Phase 3 (Solutioning)?**
A: Yes for Quick Flow. Optional for XiaoMa Method (simple projects). Required for XiaoMa Method (complex projects) and Enterprise.

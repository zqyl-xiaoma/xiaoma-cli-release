...
- **Use When:** XiaoMa Method or Enterprise track projects, integration risk is non-trivial, brownfield regression risk exists, or compliance/NFR evidence is required. (Quick Flow projects typically don't require TEA)

## TEA Workflow Lifecycle

TEA integrates into the XiaoMa development lifecycle during Solutioning (Phase 3) and Implementation (Phase 4):
...
**Phase Numbering Note:** XiaoMa uses a 4-phase methodology with optional Phase 1 and documentation prerequisite:
...
Quick Flow track skips Phase 1 and 3. XiaoMa Method and Enterprise use all phases based on project needs.

### Why TEA is Different from Other XMC Agents

TEA is the only XMC agent that operates in **multiple phases** (Phase 3 and Phase 4) and has its own **knowledge base architecture**.
...
Most XMC agents work in a single phase:
...
TEA is the only XMC agent with its own top-level module directory (`xmc/testarch/`):
...
This architecture enables TEA to maintain consistent, production-ready testing patterns across all XiaoMa projects while operating across multiple development phases.
...
**Enable during XIAOMA installation** by answering "Yes" when prompted.
...
These cheat sheets map TEA workflows to the **XiaoMa Method and Enterprise tracks** across the **4-Phase Methodology** (Phase 1: Analysis, Phase 2: Planning, Phase 3: Solutioning, Phase 4: Implementation).

**Note:** Quick Flow projects typically don't require TEA (covered in Overview). These cheat sheets focus on XiaoMa Method and Enterprise tracks where TEA adds value.
...
### Greenfield - XiaoMa Method (Simple/Standard Work)

**Planning Track:** XiaoMa Method (PRD + Architecture)
...
### Brownfield - XiaoMa Method or Enterprise (Simple or Complex)

**Planning Tracks:** XiaoMa Method or Enterprise Method
**Use Case:** Existing codebases - simple additions (XiaoMa Method) or complex enterprise requirements (Enterprise Method)
...
**Planning Track:** Enterprise Method (XiaoMa Method + extended security/devops/test strategies)
**Use Case:** New enterprise projects with compliance, security, or complex regulatory requirements

**🏢 Enterprise Deltas from XiaoMa Method:**
...
**To disable**: Set `tea_use_mcp_enhancements: false` in `{xiaoma_folder}/xmc/config.yaml` OR remove MCPs from IDE config.
...
**Enable during XIAOMA installation** by answering "Yes" when prompted, or manually set `tea_use_playwright_utils: true` in `{xiaoma_folder}/xmc/config.yaml`.

**To disable**: Set `tea_use_playwright_utils: false` in `{xiaoma_folder}/xmc/config.yaml`.

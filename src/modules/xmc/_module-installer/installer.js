...
async function install(options) {
  ...
  try {
    logger.log(chalk.blue('🚀 Installing XMC Module...'));
    ...
    if (installedIDEs && installedIDEs.length > 0) {
      logger.log(chalk.cyan(`Configuring XMC for IDEs: ${installedIDEs.join(', ')}`));

      // Add any IDE-specific XMC configurations here
      for (const ide of installedIDEs) {
        await configureForIDE(ide, projectRoot, config, logger);
      }
    }

    logger.log(chalk.green('✓ XMC Module installation complete'));
    return true;
  } catch (error) {
    logger.error(chalk.red(`Error installing XMC module: ${error.message}`));
    return false;
  }
}

/**
 * Configure XMC module for specific platform/IDE
 * @private
 */
async function configureForIDE(ide, projectRoot, config, logger) {
  // Validate platform code
  if (!platformCodes.isValidPlatform(ide)) {
    logger.warn(chalk.yellow(`  Warning: Unknown platform code '${ide}'. Skipping XMC configuration.`));
    return;
  }
  ...
  try {
    if (await fs.pathExists(platformSpecificPath)) {
      ...
      logger.log(chalk.dim(`  No XMC-specific configuration for ${platformName}`));
    }
  } catch (error) {
    logger.warn(chalk.yellow(`  Warning: Could not load XMC platform-specific handler for ${platformName}: ${error.message}`));
  }
}

...
**Workflow ID**: `{xiaoma_folder}/xmc/testarch/test-design`
...
1. **Check for sprint-status.yaml**
   - If `{output_folder}/xmc-sprint-status.yaml` exists → **Epic-Level Mode** (Phase 4)
   - If NOT exists → Check workflow status

2. **Check workflow-status.yaml**
   - Read `{output_folder}/xmc-workflow-status.yaml`

...
1. Load relevant knowledge fragments from `{project-root}/{xiaoma_folder}/xmc/testarch/tea-index.csv`:
...
3. Read related XiaoMa artifacts (if available):
...
4. **Load supporting artifacts**:
   ...
   - `xmc-workflow-status.md` → Workflow completion status (if `check_all_workflows_complete: true`)
...
- Traceability Matrix: `{xiaoma_folder}/output/traceability-matrix.md`
- Test Design: `{xiaoma_folder}/output/test-design-epic-2.md`
- Test Results: `ci-artifacts/test-report-2025-01-15.xml`
- NFR Assessment: `{xiaoma_folder}/output/nfr-assessment-release-1.2.md`
```
...
1. **Update workflow status** (if `append_to_history: true`):
   - Append gate decision to `xmc-workflow-status.md` under "Gate History" section
   - Format:
     ...
     - **Document**: [gate-decision-story-1.3.md]({xiaoma_folder}/output/gate-decision-story-1.3.md)
...
2. **Generate stakeholder notification** (if `notify_stakeholders: true`):
   ...
   Full Report: {xiaoma_folder}/output/gate-decision-story-1.3.md
   ```
...
## Integration with XiaoMa Artifacts
...
- ✅ Workflow status updated (xmc-workflow-status.md)

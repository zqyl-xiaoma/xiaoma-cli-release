...
<critical>You MUST have already loaded and processed: {project-root}/{xiaoma_folder}/xmc/workflows/workflow-status/workflow.yaml</critical>
...
<action>Search {output_folder}/ for file: xmc-workflow-status.yaml</action>
...
<check if="no status file found">
  ...
  <check if="response == y OR response == yes">
    ...
    <invoke-workflow path="{project-root}/{xiaoma_folder}/xmc/workflows/workflow-status/init/workflow.yaml"></invoke-workflow>
...
<action>Read xmc-workflow-status.yaml</action>
...
<check if="choice == 3">
  <action>Display complete xmc-workflow-status.yaml file contents</action>
</check>
...
<action>Read {output_folder}/xmc-workflow-status.yaml if exists</action>
...
<template-output>status_file_path = {{path to xmc-workflow-status.yaml}}</template-output>
</check>
...
<action>Check if {output_folder}/xmc-workflow-status.yaml exists</action>
...
<action>Read {output_folder}/xmc-workflow-status.yaml</action>

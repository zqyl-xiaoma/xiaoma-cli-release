...
<critical>This workflow handles BOTH new projects AND legacy projects following the XiaoMa Method</critical>

<workflow>

<step n="1" goal="Scan for existing work">
<output>Welcome to XiaoMa Method, {user_name}!</output>

<action>Perform comprehensive scan for existing work:

- XMC artifacts: PRD, epics, architecture, UX, brief, research, brainstorm
...
- LEGACY: Has code but no XMC artifacts
...
To check progress: Load any XMC agent and run /xiaoma:xmc:workflows:workflow-status
...
1. **XiaoMa Method** - Full planning for complex projects
...
<output>Based on your project, here are your XiaoMa Method planning options:

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**1. XiaoMa Method** 🎯 {{#if recommended}}(RECOMMENDED){{/if}}
...
<ask>Which XiaoMa Method approach fits best?
...
<ask if="n">Which XiaoMa Method track instead?
...
<output>Your XiaoMa workflow path:
...
<check if="y">
  ...
  <action>Save to {output_folder}/xmc-workflow-status.yaml</action>
  <action>Identify next workflow and agent</action>

<output>✅ **Created:** {output_folder}/xmc-workflow-status.yaml

**Next:** {{next_workflow_name}}
**Agent:** {{next_agent}}
**Command:** /xiaoma:xmc:workflows:{{next_workflow_id}}

{{#if next_agent not in [analyst, pm]}}
💡 Start new chat with **{{next_agent}}** agent first.
{{/if}}

To check progress: /xiaoma:xmc:workflows:workflow-status

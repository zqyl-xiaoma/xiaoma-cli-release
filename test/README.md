...
Comprehensive test coverage for the XIAOMA agent schema validation system.

## Overview

This test suite validates the Zod-based schema validator (`tools/schema/agent.js`) that ensures all `*.agent.yaml` files conform to the XIAOMA agent specification.
...
- Malformed module paths (e.g., `src/modules/xmc` without `/agents/`)
...
2. Add comment metadata at the top:
   ...
   # Path context: src/modules/xmc/agents/test.agent.yaml (if needed)
   ```
